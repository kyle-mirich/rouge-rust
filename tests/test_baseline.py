...
def test_score_batch_matches_iterative_score() -> None:
    ...
    for batch_score, iterative_score in zip(batch_scores, iterative_scores):
        ...
        for rouge_type in ROUGE_TYPES:
            assert _score_tuple(batch_score[rouge_type]) == _score_tuple(iterative_score[rouge_type])
...
def test_score_batch_flat_matches_iterative_score() -> None:
    ...
    for index, iterative_score in enumerate(iterative_scores):
        assert _flat_score_tuple(batch_scores, index, "rouge1") == _score_tuple(iterative_score["rouge1"])
        assert _flat_score_tuple(batch_scores, index, "rouge2") == _score_tuple(iterative_score["rouge2"])
        assert _flat_score_tuple(batch_scores, index, "rougeL") == _score_tuple(iterative_score["rougeL"])
